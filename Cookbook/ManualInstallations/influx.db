...
class MockInflux(object):
    def mockinstall(self):
        ...
        j.system.fs.changeDir('/opt/influxdb')
     #   return True

    # def config(self):
        j.system.fs.createDir("/opt/influxdb/cfg")   
        cfg = j.dirs.replaceTxtDirVars(CONFIG, additionalArgs={})
        j.system.fs.createEmptyFile('config.toml')
        j.do.writeFile("/opt/influxdb/cfg/config.toml", cfg)
      #  return True

    # def run(self):
        j.system.process.execute('./influxd -config=cfg/config.toml', dieOnNonZeroExitCode=True, outputToStdout=True ) 
        return True
